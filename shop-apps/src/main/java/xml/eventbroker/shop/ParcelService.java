...
public class ParcelService extends HttpServlet {
	...
	private final String myURL = "http://localhost:8080/shop-apps/ParcelService";
	...
	private final String[] state = {
			"Das Paket wird zur Zustellbasis transportiert.",
			"Das Paket wird heute ausgeliefert.", "Das Paket wurde zugestellt." };
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse resp)
		...
		for (Integer id : parcels.keySet()) {
			...
			writer.append("<b>" + id + "</b>: from:" + p.from + " to:" + p.to
					+ " Size:" + p.size + " State:" + state[p.state]);
			writer.append("</li>");
		}
	...
	private void changeParcelState(int id) {
		...
		if (parcel.state == 0) {
			parcel.state++;
			sendMessage(parcel.to, "[Parcel Service] Das Paket  " + id
					+ " wird heute ausgeliefert.");
		} else if (parcel.state == 1) {
			parcel.state++;
			sendMessage(parcel.from, "[Parcel Service] Das Paket  " + id
		...
		if (parcel.state < 2) {
			...
			String day = c.get(Calendar.YEAR) + "-" + c.get(Calendar.MONTH) + 1
					+ "-" + c.get(Calendar.DAY_OF_MONTH);
			String hour = c.get(Calendar.HOUR_OF_DAY) < 10 ? "0"
					+ c.get(Calendar.HOUR_OF_DAY) : ""
					+ c.get(Calendar.HOUR_OF_DAY);
			String minute = c.get(Calendar.MINUTE) < 10 ? "0"
					+ (c.get(Calendar.MINUTE)) : "" + (c.get(Calendar.MINUTE));
			sendEvent("<alarm-registration day='" + day + "' hour='" + hour
					+ "' minute='" + minute + "' id='" + myURL + id + "'/>");
		}
	}
	...
	private boolean addParcel(String from, String to, int size) {
		...
		int id = getID();
		parcels.put(id, p);
		sendMessage(to, "[Parcel Service] Ein Paket von " + from
				+ " mit der Trackingnummer " + id + " wurde an Sie verschickt.");
		debit(from, size, id);
		GregorianCalendar c = new GregorianCalendar();
		c.add(Calendar.MINUTE, 1);
		String day = c.get(Calendar.YEAR) + "-" + c.get(Calendar.MONTH) + 1
	...
	private void debit(String from, int size, int id) {
		String event = "<transfer to='parcelservice' from='" + from
				+ "' amount='" + getPostage(size)
				+ "' comment='Portage for parcel" + id + "' />";
		sendEvent(event);
	}
	...
	private void sendMessage(String to, String message) {
		String event = "<mail from='parcel service' to='" + to + "'>" + message
				+ "</mail>";
		sendEvent(event);
	}
