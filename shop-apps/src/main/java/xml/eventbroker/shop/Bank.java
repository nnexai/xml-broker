...
public class Bank extends HttpServlet {
	...
	private synchronized void transfer(String from, String to, Double amount,
			String comment) {
		...
		log.append("Transfer: " + from + " => " + to + ": " + amount + " ["
				+ comment + "]<br />");

		sendMessage(from, "[Ueberweisungsbeleg] An:" + to + " Betrag: -"
				+ amount + " (" + comment + ")");
		sendMessage(to, "[Ueberweisungsbeleg] Von:" + from + " Betrag: +"
				+ amount + " (" + comment + ")");
	}

	private void sendMessage(String to, String message) {
		String event = "<mail from='bank' to='" + to + "'>" + message
				+ "</mail>";
		sendEvent(event);
	}
