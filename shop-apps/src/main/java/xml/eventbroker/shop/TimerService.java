...
public class TimerService extends HttpServlet {
	...
	private final Map<String, List<String>> alarm = new HashMap<String, List<String>>();
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		TimerTask t = new TimerTask() {
			...
			public void run() {
				...
				String day = c.get(Calendar.YEAR) + "-" + c.get(Calendar.MONTH)
	...
	private synchronized void addAlarm(String day, String hour, String minute,
			String id) {
	...
	private class EventSenderThread extends Thread {
		...
		public void run() {
			try {
				...
				writer.append("<?xml version=\"1.0\" encoding=\"utf-8\"?><events>"
						+ event + "</events>");
				writer.flush();
				writer.close();
			} catch (IOException e) {
