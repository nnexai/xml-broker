	...
	protected void doGet(HttpServletRequest request, HttpServletResponse resp)
		...
		if (path != null && path.equals("/order")) {
			...
			if (size > 0) {
				...
				String orderConfitmation = "<mail from='shop' to='" + customer
						+ "'>" + mail + "</mail>";
				String portoEvent = "<parcel from='shop' to='" + customer
						+ "' size='" + size + "' />";
				String debitEvent = "<transfer to='shop' from='" + customer
						+ "' amount='" + price + "' comment='Order " + ordernr
						+ "'/>";
				writer.append(orderConfitmation + "<br>" + portoEvent + "<br>"
						+ debitEvent);
			...
			for (Product p : products) {
				writer.append("<tr>");
				writer.append("<td>" + p.name + "</td><td>" + p.price
						+ "</td><td><input type='0' size='2' value='0' name='"
						+ p.name + "'></td>");
				writer.append("</tr>");
			}
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (event.getName().equals("transfer")) {
				...
				Double amount = Double.parseDouble(event
